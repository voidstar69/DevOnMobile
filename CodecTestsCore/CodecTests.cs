    ...
    public class CodecTests
    {
        ...
        public void testGZip()
        {
            ...
            using (var outMemStream = new MemoryStream())
            {
                ...
                var output = outMemStream.ToArray();
...
//     checkStreamEncode(zipStream, outMemStream, input);
                ...
                var output = outMemStream.ToArray();
                Console.WriteLine("Deflate: {0}% ({1} bytes)", (double) output.Length / input.Length * 100,
                    output.Length);
            }
        }
...
*/
        ...
        public void testCharacterRunLengthCodecWithRandomData()
        {
            ...
            Console.WriteLine();
            Console.WriteLine("*** Compression ratio: {0}% (encoded size vs original size) ***",
                (double) totalEncodedSize / totalDecodedSize * 100);
        }
        ...
        public void testHuffmanCodecWithRandomData()
        {
            ...
            Console.WriteLine();
            Console.WriteLine("*** Compression ratio: {0}% (encoded size vs original size, in bits) ***",
                (double) totalEncodedSize / totalDecodedSize * 100);
        }
        ...
        public void testMultipleCodecs()
        {
            ...
            checkCodec(codec3, "Hello Wooorld",
                "0001101001101010011101001101100111110110001000001001000100100111101010100100110110100001011011001110101010001011111");
            checkCodec(codec3, "hhheelooo   woorrrlllld!!",
                "001111101100010010011011110111010000010000100010110101001110001100001001101001101001101101001001001101110111100000001101101101010000101101101111111111111010011001100");

            var input7 = genText(1000, 1.0);
            Console.WriteLine("Compression ratio on {0} random letters (encoded size vs original size):",
                input7.Length);
